...
NHANES_1 <- merged_NHANES_1 %>% rename(gender=DMDHRGND,education=DMDHREDU,age=DMDHRAGE,poverty_ratio=INDFMPIR,
                                   pregnant=RIDEXPRG,marital_status=DMDHRMAR,
                                   household_income=INDHHIN2,race=RIDRETH3,d1_calories=DR1TKCAL,d1_protein=DR1TPROT,
    ...
    select(SEQN,gender,education,age,poverty_ratio,household_income,race,pregnant,marital_status,d1_calories,d1_protein,d1_carb,
...
NHANES_1 <- NHANES_1 %>% rename(mex_american=race_1,hispanic=race_2,white=race_3,black=race_4,asian=race_6,muliracial=race_7) %>%
  select(-race, -gender, -household_income, -marital_status, -marital_status_77, -marital_status_99, -marital_status_NA)
